...
def get_train_dataset(p, transform=None):
    ...
    elif p['train_db_name'] == 'imaterialist-fashion-2020-fgvc7':
        from data.dataloaders.imaterialist import iMaterialist
        dataset = iMaterialist(split=p['train_db_kwargs']['split'], transform=transform)
...
def get_val_dataset(p, transform=None):
    ...
    elif p['val_db_name'] == 'imaterialist-fashion-2020-fgvc7':
        from data.dataloaders.imaterialist import iMaterialist
        dataset = iMaterialist(split='val', transform=transform)  
    else:
...
def get_val_transformations():
    return transforms.Compose([custom_tr.FixedResize(resolutions={'image': tuple((256,256)),
                                                        'semseg': tuple((256,256))},
